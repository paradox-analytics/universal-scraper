...
class ExtractedContent:
    ...
    def get_metadata_summary(self) -> str:
        ...
        if self.css_class_data:
            ...
            for key, values in self.css_class_data.items():
                ...
                if 'rating' in key.lower() or 'star' in key.lower():
                    ...
                    css_items.append("  ↳ NOTE: Use numeric values (1-5) for rating field")
                else:
                    css_items.append(f"{key}: {', '.join(unique_vals)}")
            if css_items:
                parts.append("CSS CLASS DATA (ratings, votes, scores):\n" + "\n".join(css_items))
        ...
        if self.button_data:
            ...
            if button_items:
                parts.append("BUTTON/ACTION DATA (upvotes, likes, votes):\n" + "\n".join(button_items))
