...
class AdaptiveAntiBlockingAgent:
    ...
    def __init__(
        ...
        logger.info("🤖 Adaptive Anti-Blocking Agent initialized")
    ...
    async def fetch_with_adaptation(
        ...
        logger.info("🧠 Intelligent Strategy Selected:")
        ...
        if learned_config:
            ...
            if result['success']:
                logger.info("✅ Learned config worked!")
                result['strategy_used'] = 'learned'
                return result
            else:
                logger.info("⚠️  Learned config failed, trying alternatives")

        # STEP 3: If Web Unblocker is recommended, try it
        if strategy['use_web_unblocker']:
            logger.info("🛡️  Strategy recommends Web Unblocker - trying it")
            ...
            if result['success']:
                logger.info("✅ Web Unblocker worked!")
        ...
        if preset != ConfigPreset.STEALTH:
            logger.info("⚠️  All variations failed, escalating to STEALTH preset")
            ...
            if result['success']:
                logger.info("✅ STEALTH preset worked!")
