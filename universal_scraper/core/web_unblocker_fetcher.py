    ...
    def __init__(
        ...
        if separator:
            ...
            if len(parts) >= 4:
                ...
                logger.info(" Using Native Proxy-Based Access (proxy credentials)")
                ...
                logger.warning(" Invalid proxy format, assuming API key")
        else:
            ...
            logger.info(" Using Direct API Access (Bearer token)")
        ...
        logger.info(" Web Unblocker Fetcher initialized")
        logger.info(f"   Zone: {zone}")
        if self.use_proxy_method:
            logger.info("   Method: Native Proxy-Based Access")
            logger.info(f"   Proxy: {self.proxy_host}:{self.proxy_port}")
        else:
            logger.info("   Method: Direct API Access")
                ...
                if response.status_code == 200:
                    ...
                    if len(html) < 1000:
                        ...
                        if 'kasada' in html_preview or 'kpsdk' in html_preview or 'blocked' in html_preview:
                            logger.warning("    Still appears blocked (Kasada challenge detected)")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if hasattr(self, '_fallback_username') and self._fallback_username and attempt == 0:
                        logger.warning("    Password part failed, trying username part as API key...")
                    ...
                    logger.error(f"    {error_msg}")
                    logger.error("    Tip: Web Unblocker API key is different from proxy credentials.")
                    logger.error("    Get your API key from: https://brightdata.com/cp/account/api")
                    logger.error("    The API key should be a Bearer token, not proxy credentials.")
                    raise Exception(error_msg)
