...
from .extraction_pattern import ExtractionPattern, PatternType, PatternGenerator, PatternExecutor
...
class SmartPatternCache:
    ...
    def __init__(
        ...
        logger.info(f"🧠 Smart Pattern Cache initialized")
            ...
            if pattern_data:
                ...
                if self._is_pattern_valid(pattern):
                    ...
                    if await self._validate_pattern_on_html(pattern, html, fields):
                        ...
                        logger.info(f"⚠️ Domain pattern exists but failed validation, will regenerate")
        ...
        for pattern in all_patterns:
            ...
            if pattern_fields_set.issubset(requested_fields_set):
                ...
                if matched_count > best_match_count:
                    ...
                    logger.info(f"  🔍 Validating pattern on current HTML...")
                    if await self._validate_pattern_on_html(pattern, html, list(pattern_fields_set)):
                        ...
                        logger.info(f"  ✅ Pattern validated successfully!")
                    else:
                        logger.warning(f"  ⚠️ Pattern validation failed")
            else:
...
class SmartScraper:
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"🚀 SmartScraper initialized with pattern caching")
    ...
    async def scrape(
        ...
        if pattern:
            ...
            if success:
                ...
                logger.info(f"⚠️ Cached pattern failed, falling back to Direct LLM")
        
        # Step 2b: No pattern or pattern failed - use Direct LLM
        logger.info(f"🤖 Using Direct LLM extraction (will learn pattern)")
