...
class JSONDetector:
    ...
    def detect_and_extract(self, html: str, url: str, captured_json: Optional[List] = None, force_json_ld: bool = False) -> Dict[str, Any]:
        ...
        if force_json_ld:
            ...
            if json_ld_data:
                logger.info(" Found JSON-LD (forced by cached strategy)")
                ...
                if not fields:
                    ...
                    if isinstance(data, dict):
                        ...
                        if items:
                            ...
                            if is_newsletter:
                                logger.debug("   ⏭  Skipping single item (newsletter/subscription content)")
                            elif is_generic_metadata and not is_rich_content:
                                logger.debug("   ⏭  Skipping single item (schema.org metadata)")
                            else:
    ...
    def _find_item_arrays(self, data: Any, max_depth: int = 10, current_depth: int = 0) -> List[Dict]:
        ...
        if candidates:
            ...
            if best_score < 0:
                ...
                logger.info("    Rejecting all arrays - likely breadcrumbs/navigation")
                return []
    ...
    def _extract_fields_semantically(self, items: List[Dict], fields: List[str], llm_field_mappings: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if items and len(items) > 0:
            ...
            if len(single_char_keys) >= 5:
                ...
                if product_items:
                    ...
                    logger.warning("     All items filtered out! Using original items for inference.")
        ...
        for idx, item in enumerate(items):
            ...
            if extracted_item:
                ...
                logger.warning("    First item had ZERO field matches!")
        ...
        if extracted:
            ...
            logger.warning("    NO items extracted! All fields failed to match.")
    ...
    def _infer_key_mapping(self, items: List[Dict], fields: List[str]) -> Dict[str, str]:
                    ...
                    if 'str' in stats['types'] and stats['lengths']:
                        ...
                        if 5 <= avg_len <= 100 and stats['has_url'] < sample_size * 0.3:
                            ...
                            for sample in stats['samples']:
                                if sample and sample.lower() not in ['listitem', 'home']:
                                    score += 5
        ...
        if not mapping:
            logger.warning("    NO key mappings inferred! Field matching failed.")
            # Debug: show key statistics
            logger.warning("    Available keys and their characteristics:")
