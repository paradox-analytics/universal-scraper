...
class HybridScraper:
    ...
    async def scrape(self, url: str, fields: List[str]) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    if extracted_items and len(extracted_items) >= 3:
                        ...
                        if is_sufficient:
                            ...
                            logger.info(f"     JSON rejected: Poor content quality (likely analytics/tracking)")
            ...
            pattern_cost = 0.0
            used_cache = False
            containers = None  # Initialize for later use in extraction
            ...
            if similar:
                ...
                best_pattern = dom_patterns.get('best_pattern') if dom_patterns else None
                
                # Extract containers from best pattern
                containers = []
                container_samples = []
                if best_pattern:
                    ...
                    if selector:
                        ...
                        logger.info(f"       Found {len(containers)} containers via {selector[:50]}")
                        
                        # Create serializable container samples
                ...
                try:
                    pattern = await self.pattern_gen.generate_pattern(
                        ...
                        repeating_containers=container_samples if container_samples else None
                    )
                    pattern_cost = 0.02
            ...
            # Use containers from DOM pattern detection if available, else fallback
            if not containers:
                # Fallback: Simple container detection
                ...
                logger.info(f"   Using {len(containers) if containers else 0} fallback containers")
            else:
