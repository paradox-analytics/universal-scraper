    ...
    if APIFY_AVAILABLE:
        async with Actor:
            ...
            if not api_key:
                ...
                # Process URLs anyway but with clear messaging (for testing Apify's 5-minute requirement)
                logger.info(" Processing URLs with limited functionality (API key required for full features)...")
                ...
                for i, url in enumerate(url_list, 1):
                    ...
                    # Push error message per URL
                    await Actor.push_data({
                        '_url': url,
                        '_message': 'OpenAI API key required',
                        '_error': 'This actor requires an OpenAI API key. Please add "openaiApiKey" to actor input.',
                        '_success': False,
                        '_action_required': 'Add openaiApiKey to actor input'
                    })
                ...
                await Actor.set_value('OUTPUT_METADATA', {
                    'status': 'completed',
                    'message': 'OpenAI API key required for full functionality',
                    'total_urls': len(url_list) if url_list else 0,
                    'successful': 0,
                    'total_items': 0,
                ...
                logger.info(" Note: OpenAI API key required for actual data extraction")
                logger.info("=" * 80)
            ...
            scraper = UniversalScraper(
                ...
                web_unblocker_api_key=web_unblocker_api_key,  # NEW: Web Unblocker support
