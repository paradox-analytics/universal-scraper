...
Inspired by the LLM-based extraction project's successful Camoufox implementation
...
def _camoufox_fetch_sync(
 ...
 # Inject advanced anti-detection scripts (from LLM-based extraction project)
...
class CamoufoxFetcher:
 ...
 proxy_manager: Optional['ProxyManager'] = None, # ProxyManager for rotation
 headless: bool = True,
 timeout: int = 60000,
 enable_js: bool = True,
 anti_detection_profile: str = 'random', # Anti-detection profile
 humanize: bool = True, # Enable human-like behavior
 stealth_mode: bool = True # Maximum stealth (slower but harder to detect)
 ):
 """
 ...
 self.proxy_manager = proxy_manager # For per-request rotation
 ...
 # Get fresh proxy for THIS request (Oxylabs approach)
 ...
 wait_time,
 scroll_to_bottom,
 self.anti_detection_config # Pass anti-detection config
 )
