...
Based on analysis of direct LLM extraction and Oxylabs approaches.
"""
...
class ExtractedContent:
 ...
 labeled_numbers: Dict[str, List[str]] = field(default_factory=dict) # "5 answers" -> answers: [5]
 script_data: List[str] = field(default_factory=list) # window/document variable data (direct LLM extraction style)
 button_data: Dict[str, List[str]] = field(default_factory=dict) # Upvote/action button values
 source_url: str = ""
 ...
 # Include script data (window/document variables) - direct LLM extraction style
 if self.script_data:
...
class HybridMarkdownExtractor:
 ...
 # Stage 1.5: Extract script data (direct LLM extraction style - window/document variables)
 ...
 script_data=script_data, # direct LLM extraction style
 ...
 """
 Extract data from script tags (direct LLM extraction approach).
 ...
 # direct LLM extraction specifically looks for these
 ...
 # If script is small, include it as raw content (direct LLM extraction does this)
 if len(content) < 500:
