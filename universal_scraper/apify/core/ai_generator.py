...
# Optional: HTML to Markdown conversion (like direct LLM extraction)
try:
 import html2text
...
class AICodeGenerator:
 ...
 field_hints: Optional[Dict[str, Dict[str, Any]]] = None # Semantic field mappings
 ) -> Dict[str, Any]:
 """
 Generate BeautifulSoup extraction code with multi-iteration refinement

 IMPROVEMENTS:
 1. Uses structure_analysis to guide generation (from direct LLM extraction)
 2. Multi-iteration refinement with error feedback (from direct LLM extraction)
 ...
 field_hints=field_hints # Pass semantic hints
 )
 ...
 # : Check for single-item extraction when multiple expected (SINGLE ITEM CHECK)
 ...
 """
 # OPTIONAL: Convert HTML to Markdown (direct LLM extraction approach)
 ...
 structure_analysis,
 previous_errors,
 field_hints # Semantic field mappings
 )
 ...
 Based on LLM-based extraction's proven few-shot approach

 NOW SUPPORTS MARKDOWN (direct LLM extraction approach):
 ...
 # Build structure analysis section (NEW - from direct LLM extraction + Sibling Detection)
 structure_section = ""
 if structure_analysis:
...
"""
 ...
 # Build error feedback section (NEW - from direct LLM extraction)
 error_section = ""
...
"""
 ...
 # Build semantic field hints section
 field_hints_section = ""
 if field_hints:
