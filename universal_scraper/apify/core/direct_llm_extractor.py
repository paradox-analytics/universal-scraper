...
class DirectLLMExtractor:
 """
 Direct LLM extraction - the proven approach used by direct LLM extraction, LLM-based extraction, etc.
 ...
 Quality Modes:
 - conservative: Like direct LLM extraction (≥70% fields filled, fewer items, higher quality)
 ...
 max_tokens_per_chunk: int = 4000, # Match direct LLM extraction: smaller chunks + deduplication = better coverage
 quality_mode: str = "balanced",
 use_html2text: bool = True, # Convert HTML to text (direct LLM extraction approach)
 use_hybrid_extraction: bool = True, # Use hybrid extraction (captures data attrs, forms)
 ...
 url: Optional[str] = None # URL for cache key generation
 ) -> List[Dict[str, Any]]:
 """
 ...
 ENHANCED (from direct LLM extraction + LLM-based extraction analysis):
 ...
 # OPTIMIZATION 1: Skip chunking for small pages (direct LLM extraction approach)
 ...
 # Chunk HTML with OVERLAP (LLM-based extraction-style: 33% overlap helps capture truncated items)
 OVERLAP_FACTOR = 3 # 33% overlap like LLM-based extraction
 ...
 all_items = []
 previous_items = [] # Track previous chunk's items for context (LLM-based extraction-style)
 ...
 # OPTIMIZATION: Larger batch size for better parallelism (direct LLM extraction uses 10-20)
 ...
 # direct LLM extraction's secret: DEDUPLICATE items from multiple chunks!
 ...
 url: Optional[str] = None # URL for field context
 ) -> List[Dict[str, Any]]:
 """
...
Return complete JSON with 'items' array containing the same number of items."""
 ...
 """
 Split HTML into overlapping chunks (LLM-based extraction-style).
 ...
 context: Optional[str],
 quality_mode: str,
 previous_items: Optional[List[Dict[str, Any]]] = None, # LLM-based extraction-style context passing
 url: Optional[str] = None # URL for field context
 ) -> List[Dict[str, Any]]:
 """
 ...
 LLM-based extraction-STYLE: Optionally receives previous_items from earlier chunks
 ...
 previous_items=previous_items, # LLM-based extraction-style: pass previous chunk's items
 url=url # Pass URL for field context
 )
 ...
 # Post-extraction quality filtering (now more lenient to match direct LLM extraction)
 filtered_items = self._filter_quality_items(items, fields, quality_mode)
 ...
 """
 Deduplicate items extracted from multiple chunks (direct LLM extraction approach)
 ...
 - conservative: ≥70% fields filled (like direct LLM extraction)
 ...
 # Only reject OBVIOUS navigation/UI keywords (very conservative to match direct LLM extraction)
 # Only full-match exact phrases, not substrings
 nav_keywords = [
 # Only the most obvious UI elements that would NEVER be content
 ] # Empty for now - direct LLM extraction doesn't filter aggressively
 ...
 Improves data quality by returning properly typed values like direct LLM extraction.
 ...
 url: Optional[str] = None # URL for website-specific field context
 ) -> str:
 """
 ...
 previous_items: Items extracted from previous chunks (LLM-based extraction-style continuity)
 ...
 # Build the extraction question (direct LLM extraction style: simple and direct)
 field_list = ', '.join(fields)

 # Add website-specific field context (critical for correct extraction)
 ...
 # Simple prompt like direct LLM extraction (their secret is SIMPLICITY!)
 ...
 # Enhanced guidance for retry (direct LLM extraction "reasoning" step inspired)
 if enhanced_prompt:
 prompt_parts.extend([
 "",
 ...
 # LLM-based extraction-STYLE: Add previous items context if available
 ...
 """
 Get system prompt that guides LLM behavior (direct LLM extraction approach: keep it simple!)
...
"""
 ...
 if enhanced:
 # Enhanced prompt inspired by direct LLM extraction's "reasoning" node
 return base_prompt + """
