...
from .field_mapper import UniversalFieldMapper # Semantic field mapping
from .smart_sampler import SmartHTMLSampler # Dynamic HTML sampling
from .adaptive_dom_detector import AdaptiveDOMDetector # Reinforcement-style iteration
from .embedding_cache import EmbeddingBasedSelectorCache # ML-based learning
from .direct_llm_extractor import DirectLLMExtractor # Direct LLM extraction (like direct LLM extraction)
...
class UniversalScraper:
 ...
 use_camoufox: bool = True, # Use Camoufox for better anti-detection (recommended)
 ...
 extraction_context: Optional[Any] = None, # User's extraction goal (context-driven scraping)
 enable_context_validation: bool = True, # Enable LLM validation of extracted data
 use_direct_llm: bool = True, # Use Direct LLM extraction as primary method (like direct LLM extraction)
 quality_mode: str = "balanced", # Quality mode for DirectLLM ('conservative', 'balanced', 'aggressive')
 web_unblocker_api_key: Optional[str] = None, # Bright Data Web Unblocker API key (fallback for Kasada)
 web_unblocker_zone: str = "web_unlocker1", # Web Unblocker zone name
 ...
 use_direct_llm: Use Direct LLM extraction as primary method (recommended, like direct LLM extraction)
 quality_mode: Quality mode for Direct LLM extraction:
 - 'conservative': Like direct LLM extraction (≥70% fields, fewer items, highest quality)
 ...
 # Create ProxyManager for per-request rotation (Oxylabs approach)
 proxy_manager = None
 if proxy_config:
 ...
 proxy_config=proxy_config,
 proxy_manager=proxy_manager, # Pass ProxyManager
 enable_cache=enable_cache,
 enable_warming=enable_warming,
 cache_dir=cache_dir,
 headless=headless,
 browser_timeout=browser_timeout,
 use_camoufox=use_camoufox, # Pass Camoufox preference
 web_unblocker_api_key=web_unblocker_api_key, # Web Unblocker fallback
 ...
 # Initialize Direct LLM Extractor (NEW: Like direct LLM extraction)
 ...
 self.enable_auto_pagination = enable_auto_pagination # Control auto-pagination behavior
 ...
 self.html_structure_analyzer = None # From direct LLM extraction
 self.json_structure_analyzer = None # JSON structure analysis (like HTML)
 ...
 # Initialize HTML Structure Analyzer (NEW: From direct LLM extraction)
 if api_key:
 ...
 **scraper_kwargs: Additional UniversalScraper init arguments
 ...
 original_html = html # Store original HTML for Direct LLM supplementation
 ...
 fields,
 wait_for_selector=wait_for_selector,
 pre_warmed_mappings=pre_warmed_mappings # Pass pre-warmed mappings for optimization
 )
 else:
 ...
 try:
 # Since pagination uses JSON extraction, prefer JSON data over HTML
...
Note: Look for product names/titles in fields like 'name', 'title', 'productName', 'product_name', or similar fields."""
 ...
 # Analyze JSON structure first (like HTML structure analysis)
 ...
 try:
 # Pass LLM-discovered field mappings to extraction (universal fallback)
 ...
 'early_exit': True, # Flag for early exit
 'timestamp': time.time()
 },
 ...
 # OPTIMIZATION: Clean HTML BEFORE chunking (direct LLM extraction approach)
 ...
 fields=fields,
 context=context_str,
 url=url # Pass URL for cache key generation
 )
 else:
 direct_llm_items = self.direct_llm_extractor.extract(
 html=cleaned_html, # CLEANED HTML - reduces chunks dramatically
 ...
 # Step 5.5: Analyze HTML structure (NEW: From direct LLM extraction)
 ...
 structure_analysis=structure_analysis, # !
 max_iterations=3, # Multi-iteration refinement
 field_hints=field_hints # Semantic field mappings
 )
 ...
 pre_warmed_mappings: Optional[Dict[str, Any]] = None # Pre-warmed field mappings (for optimization)
 ) -> List[Dict[str, Any]]:
 """
 ...
 all_data = []
 first_page_html = None # Store HTML from first page for Direct LLM supplementation
 first_page_json_data = None # Store JSON data from first page for Direct LLM supplementation
 ...
 # Return HTML and JSON data from first page for Direct LLM supplementation
 batch_tasks = [
 ...
 return all_data, first_page_html, first_page_json_data # Return HTML and JSON data from first page
 ...
 wait_for_selector: Optional[str] = None,
 return_html: bool = False, # Optionally return HTML for Direct LLM supplementation
 return_json_data: bool = False, # Optionally return raw JSON data for Direct LLM supplementation
 pre_warmed_mappings: Optional[Dict[str, Any]] = None # Pre-warmed field mappings (for pagination optimization)
 ) -> List[Dict[str, Any]]:
 """
 ...
 **kwargs: Additional arguments passed to scrape()
 ...
 This is the direct LLM extraction approach: Convert to Markdown + Direct LLM extraction
...
"""
 ...
 fields=fields,
 domain=domain,
 sibling_analysis=sibling_analysis # - enables context-block extraction
 )
