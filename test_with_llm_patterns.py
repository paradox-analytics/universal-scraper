"""
Test Hybrid System with LLM Pattern Generation

Demonstrates the full capability of the hybrid scraper with:
...
# Test sources - focusing on sites that work well with LLM extraction
TEST_SOURCES = [
    {
...
class LLMPatternTest:
    """Test LLM-powered pattern generation."""
    ...
    def __init__(self, api_key: str):
        ...
        logger.info("🚀 LLM Pattern Test initialized with API key")
    ...
    async def scrape(self, url: str, fields: list, category: str, name: str):
        ...
        try:
            ...
            if similar:
                ...
                # CACHE MISS - Generate pattern with LLM
                ...
                logger.info(f"      🤖 Calling GPT-4o-mini to generate semantic pattern...")
                ...
                try:
                    ...
                    logger.info(f"      📋 Generated Pattern Preview:")
                    for field_name, field_pattern in list(pattern.items())[:2]:
                        primary = field_pattern.get('primary', {})
                        logger.info(f"         • {field_name}: {primary.get('type', 'unknown')}")
            ...
            if 'hacker' in url.lower():
                ...
                containers = soup.find_all('div', class_=lambda x: x and 'post-summary' in x)[:20]
            ...
            status_icon = "✅" if used_llm else ("♻️" if used_cache else "⚠️")
            ...
            return {
                ...
                "cost": pattern_cost,
                "time": total_time,
                "data": extracted_data[:10]  # Save first 10 for inspection
            }
    ...
    def print_summary(self, results):
        """Print test summary."""
        logger.info(f"\n\n{'='*80}")
        logger.info(f"📊 TEST SUMMARY - LLM PATTERN GENERATION")
        logger.info(f"{'='*80}")
...
async def main():
    """Run LLM pattern generation test."""
    logger.info("="*80)
    logger.info("🧪 HYBRID SYSTEM TEST - LLM PATTERN GENERATION")
    logger.info("="*80)
    logger.info(f"\nTesting {len(TEST_SOURCES)} websites with LLM-powered patterns")
    logger.info("Demonstrating high-quality extraction and caching\n")
    
    tester = LLMPatternTest(api_key=API_KEY)
    results = []
    ...
    output_file = f"llm_pattern_results_{timestamp}.json"
    ...
    if tester.metrics['llm_calls'] > 0:
        logger.info(f"\n✅ LLM Pattern Generation WORKING!")
        logger.info(f"   Generated {tester.metrics['llm_calls']} semantic patterns")
        logger.info(f"   Cost: ${tester.metrics['total_cost']:.4f} total")
    ...
    logger.info(f"\n✅ Hybrid system validated with LLM patterns! 🎉")
