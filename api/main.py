...
# The agent and browser endpoint implementations live in:
#   api/routes/agents.py  — agent CRUD, scheduling, execution
#   api/routes/browser.py — browser session management, live preview
...
async def execute_agent_task(agent_id: str, api_key: Optional[str] = None):
    ...
    try:
        ...
        if agent.type == AgentType.WEB_SCRAPING:
            ...
            result = await scraper.scrape(
                url=config.get("url"), fields=config.get("fields", []),
            ...
            if result.get("data"):
                ...
                await pattern_cache.store_pattern(
                    tenant_id=agent.tenant_id, domain=domain,
                    fields=config.get("fields", []),
                    pattern_data={"source": result.get("source"), "item_count": len(result.get("data", []))},
                    visibility=CacheVisibility.PRIVATE, url=config.get("url"),
                )
